...
def handler(req):
    """
    Handler for Apache mod_python
    :param req: Apache request
    :return:  Apache response
    """
    ...
    lines = log.readlines()[start if start != 0 else None:stop if stop != 0 else None]

    req.content_type = "text/html"
    req.write("""<!DOCTYPE HTML><html lang="en"><head><meta charset="utf-8" />
        <link href="css/bootstrap.css" rel="stylesheet"></head><body>
        <div class="container" style="max-width: 900px;">""")
    req.write("<p>start: %d, stop: %d</p>" % (start, stop))
    ...
    req.write("""<table class=\"table table-condensed table-striped\">
        <thead><tr><th>full time</th><th>operation</th><th>user login</th><th>user id</th>
        <th>file name</th><th>comment</th></tr></thead><tbody>""")
    for line in lines:
